...
fn emitPointerMotion(cursor: *Cursor, event: *const Seat.Event.PointerMotionRelative) void {
    Compositor.notify(.{ .pointer_motion = .{
        .seat = cursor.seat,
        .time_msec = event.time_msec,
        .x = cursor.wlr_cursor.x,
        .y = cursor.wlr_cursor.y,
        .dx = event.delta_x,
        .dy = event.delta_y,
...
pub fn processMotionRelative(cursor: *Cursor, event: *const Seat.Event.PointerMotionRelative) void {
    ...
    cursor.emitPointerMotion(event);
...
fn windowAtCursor(cursor: *Cursor) ?*Window {
    if (server.scene.at(cursor.wlr_cursor.x, cursor.wlr_cursor.y)) |result| {
        switch (result.data) {
...
pub fn processButton(cursor: *Cursor, event: *const Seat.Event.PointerButton) void {
    if (event.state == .pressed) {
        ...
        if (cursor.seat.matchPointerBinding(event.button)) |binding| {
            ...
            binding.pressed();
            // Emit pointer_button even for bindings so compositor sees all clicks.
        ...
        // Hardware presses are always .normal; decoration-initiated move/resize
        // is signaled via a separate synthesized pointer_button from
        // XdgToplevel.handleRequestMove/Resize with kind=.move/.resize.
        const win_at = cursor.windowAtCursor();
        const pending_kind: Compositor.PointerButtonKind = .normal;
        const pending_edges: Window.Edges = .{};
            ...
            .passthrough => {
                if (server.scene.at(cursor.wlr_cursor.x, cursor.wlr_cursor.y)) |at| {
                    ...
                    if (at.surface != null) {
                        ...
                        cursor.emitPointerButton(event, pending_kind, pending_edges, win_at);
                        return;
                    }
                }
