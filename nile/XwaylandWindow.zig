...
pub fn handleMap(listener: *wl.Listener(void)) void {
    ...
    // Nile: ensure every new window emits window_add so layout sees it.
    // XDG does this via handleCommit initial_commit; Xwayland skipped it
    // before, so tiling compositors (e.g. NileCompositor) never saw
    // Xwayland windows. Keep window_add emission centralized here without
    // changing NileCompositor's tiling logic.
    Compositor.notify(.{ .window_add = window });
    window.map() catch {
        log.err("out of memory", .{});
        surface.resource.getClient().postNoMemory();
    };
