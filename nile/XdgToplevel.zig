...
fn handleCommit(listener: *wl.Listener(*wlr.Surface), _: *wlr.Surface) void {
    ...
    if (toplevel.wlr_toplevel.base.initial_commit) {
        ...
        window.state = .ready;
        @import("Compositor.zig").notify(.{ .window_add = window });
        server.wm.dirtyWindowing();
        return;
    }
...
fn handleRequestMaximize(listener: *wl.Listener(void)) void {
    ...
    if (toplevel.wlr_toplevel.requested.maximized) {
        ...
        @import("Compositor.zig").notify(.{ .window_maximize_request = .{ .window = toplevel.window, .maximize = true } });
    } else {
        toplevel.window.wm_scheduled.maximize_requested = .unmaximize;
        @import("Compositor.zig").notify(.{ .window_maximize_request = .{ .window = toplevel.window, .maximize = false } });
    }
    server.wm.dirtyWindowing();
}
...
fn handleRequestMinimize(listener: *wl.Listener(void)) void {
    ...
    @import("Compositor.zig").notify(.{ .window_minimize_request = toplevel.window });
    server.wm.dirtyWindowing();
}
...
fn handleRequestMove(
    ...
    if (seat.wlr_seat.validatePointerGrabSerial(null, event.serial)) {
        ...
        // Synthesize a pointer_button with move kind so the compositor can
        // distinguish a moving hold from a normal hold.
        var pressed_btn: u32 = 272; // BTN_LEFT fallback
        var it = seat.cursor.pressed.iterator();
        while (it.next()) |kv| {
            pressed_btn = kv.key_ptr.*;
            break;
        }
        @import("Compositor.zig").notify(.{ .pointer_button = .{
            .seat = seat,
...
fn handleRequestResize(listener: *wl.Listener(*wlr.XdgToplevel.event.Resize), event: *wlr.XdgToplevel.event.Resize) void {
    ...
    if (seat.wlr_seat.validatePointerGrabSerial(null, event.serial)) {
        const edges: Window.Edges = @bitCast(event.edges);
        toplevel.window.wm_scheduled.pointer_resize_requested = .{
            .seat = seat,
            .edges = edges,
        };
        var pressed_btn: u32 = 272;
        var it = seat.cursor.pressed.iterator();
        while (it.next()) |kv| {
            pressed_btn = kv.key_ptr.*;
            break;
...
fn handleSetParent(listener: *wl.Listener(void)) void {
    ...
    @import("Compositor.zig").notify(.{ .window_parent_changed = toplevel.window });
    server.wm.dirtyWindowing();
}
