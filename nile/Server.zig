...
pub fn init(server: *Server, runtime_xwayland: bool) !void {
    ...
    const backend = wlr.Backend.autocreate(loop, &session) catch blk: {
        // `autocreate` respects WLR_BACKENDS. If it failed (e.g. WAYLAND_DISPLAY
        // points at a deleted socket - `rm /run/user/1000/wayland-*` will do this;
        // see `ss -x | grep wayland` still shows ESTAB but filesystem entry gone)
        // Try X11 (DISPLAY=:0 via XWayland on KDE) before headless, so a visible
        // window still opens. Headless has no visible output.
        if (std.c.getenv("DISPLAY")) |dpy_raw| {
            log.warn("autocreate failed, trying X11 backend on {s} (outer WAYLAND_DISPLAY={s} missing?)", .{ std.mem.sliceTo(dpy_raw, 0), if (std.c.getenv("WAYLAND_DISPLAY")) |v| std.mem.sliceTo(v, 0) else "(unset)" });
            if (wlr.Backend.createX11(loop, dpy_raw)) |x11| {
                // X11 backend must be wrapped in a multi backend for wlr_multi_for_each_backend
                if (wlr.Backend.createMulti(loop)) |multi| {
                    if (multi.multiAdd(x11)) {
                        session = null;
                        break :blk multi;
                    } else {
                        log.warn("multiAdd X11 failed, trying headless", .{});
                        multi.destroy();
                        x11.destroy();
                    }
                } else |e| {
                    log.warn("createMulti for X11 failed: {s}, trying headless", .{@errorName(e)});
                    x11.destroy();
                }
            } else |e| {
                log.warn("X11 backend failed: {s}, trying headless", .{@errorName(e)});
            }
        } else {
            log.warn("autocreate failed and no DISPLAY, falling back to headless - set WLR_BACKENDS to override", .{});
        }
        {
            const xdgrd = std.c.getenv("XDG_RUNTIME_DIR");
            const path = if (xdgrd) |v| std.mem.sliceTo(v, 0) else "/run/user/1000";
            log.warn("headless has no visible window - clients can still connect to {s}/wayland-1 and windows will tile (use WLR_BACKENDS=x11 for nested X11 window or restart session to restore wayland-0)", .{path});
        }
        session = null;
        const hb = wlr.Backend.createHeadless(loop) catch |e| {
            log.err("headless fallback also failed: {s}", .{@errorName(e)});
            return e;
        };
        _ = hb.headlessAddOutput(1280, 720) catch |e| {
            log.err("failed to add headless output: {s}", .{@errorName(e)});
            hb.destroy();
            return e;
        };
        break :blk hb;
