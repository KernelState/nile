...
> **Nile fork:** This is [Nile](https://github.com/KernelState/nile) — River reimagined as a **function-first compositor**. The six custom River Wayland protocols (`river_window_management_v1`, `river_xkb_bindings_v1`, `river_layer_shell_v1`, `river_input_management_v1`, `river_libinput_config_v1`, `river_xkb_config_v1`) have been **removed**. All window management is now via direct Zig functions in `river/Nile.zig`. See [`doc/nile-api.md`](doc/nile-api.md) for the new API.
...
- River's six custom `river_*_v1` protocols and their XML files (`protocol/river-*.xml`) have been deleted. No `river-protocols` pkgconfig is installed.

For River classic protocol design (historical), see [Separating the Wayland Compositor and Window Manager](https://isaacfreund.com/blog/river-window-management/).
