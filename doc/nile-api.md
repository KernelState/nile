...
> **Status:** Transitional. The Wayland globals `river_window_manager_v1`, `river_xkb_bindings_v1`, `river_layer_shell_v1`, `river_input_management_v1`, `river_libinput_config_v1`, `river_xkb_config_v1` are still generated but **not advertised** by default. They will be removed in a future release. New code must use `river/Nile.zig`.
...
Standard Wayland / wlroots protocols (`xdg_shell`, `wlr_layer_shell`, `ext_session_lock`, etc.) are **unchanged**. Only the six `river_*` protocols are deprecated.
...
If you need compatibility with an existing external window manager (e.g. `tinyrwm`), set:

```zig
Nile.enableLegacyProtocols = true;
server.wm.global = try wl.Global.create(...);
server.xkb_bindings.global = try wl.Global.create(...);
// etc. — see commented lines in WindowManager.zig, XkbBindings.zig, ...
```

Or toggle at build time with `-Dlegacy-protocols=true` (future build option). By default, the globals are `null` and not advertised.
